...
contract('Test Advanced Properties', async (accounts) => {
    ...
    it('Agreement 1.1 returns name', async () => {

        let nameGot = await agreement.getName.call();
        assert.equal(nameGot[0], name[0], "Agreement 1.1 doesn't return name (0)");
        assert.equal(nameGot[1], name[1], "Agreement 1.1 doesn't return name (1)");

    })

    it('Agreement 1.1 returns descyption', async () => {

        let descriptionGot = await agreement.getDescription.call();
        assert.equal(descriptionGot[0], description[0], "Agreement 1.1 doesn't return descryption (0)");
        assert.equal(descriptionGot[1], description[1], "Agreement 1.1 doesn't return descryption (1)");
        assert.equal(descriptionGot[2], description[2], "Agreement 1.1 doesn't return descryption (2)");
        assert.equal(descriptionGot[3], description[3], "Agreement 1.1 doesn't return descryption (3)");
        assert.equal(descriptionGot[4], description[4], "Agreement 1.1 doesn't return descryption (4)");
        assert.equal(descriptionGot[5], description[5], "Agreement 1.1 doesn't return descryption (5)");
        assert.equal(descriptionGot[6], description[6], "Agreement 1.1 doesn't return descryption (6)");
        assert.equal(descriptionGot[7], description[7], "Agreement 1.1 doesn't return descryption (7)");

    })

    it('Agreement 1.1 JSON ABI', async () => {
        let testJSON = [
